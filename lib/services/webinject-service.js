...
var Service = function(params) {
  ...
  var interceptUrls = [];
  var headInjectedCode = '';
  var bodyInjectedCode = '';
  ...
  var checkSkipInterception = function() {
    return (lodash.isEmpty(headInjectedCode) && lodash.isEmpty(bodyInjectedCode));
  }

  var checkHeadNotEmpty = function() {
    return !lodash.isEmpty(headInjectedCode);
  }

  var checkBodyNotEmpty = function() {
    return !lodash.isEmpty(bodyInjectedCode);
  }
  ...
  self.buildInjectorRouter = function(express) {
    ...
    if (pluginCfg.interceptor == 'tamper') {
      router.use(interceptUrls, tamper(function(req, res) {
        ...
        if (checkSkipInterception()) return;
      ...
      router.use(interceptUrls, interceptor(function(req, res) {
        return {
          isInterceptable: function(){
            return (/text\/html/.test(res.get('Content-Type')) && !checkSkipInterception());
          },
